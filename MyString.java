...
public class MyString {
    ...
    public static long NthFibonacci(int numberCount) {
        if (numberCount == 0) {
            return 0;
        }
        ...
        return NthFibonacci(numberCount - 1) + NthFibonacci(numberCount - 2);
    }
    /***
    ...
    public static int countConsonants(String inputString, int indexCount) {
        if (indexCount >= inputString.length()) {
            return 0;
        }
        char character = Character.toLowerCase(inputString.charAt(indexCount));
        if ((character >= 'a' && character <= 'z') && !(character == 'a' || character == 'e' || character == 'i' || character == 'o' || character == 'u')) {
            return 1 + countConsonants(inputString, indexCount + 1);
        }
        else {
            return countConsonants(inputString, indexCount + 1);
        }
    }
    /***
    ...
    public static String toSnakeCase(String input, int indexCount) {
        if (indexCount >= input.length()) {
            return "";
        }
        char character = input.charAt(indexCount);
        if (character == ' ') {
            return "_" + toSnakeCase(input, indexCount + 1);
        }
        else if (Character.isUpperCase(character)) {
            return "_" + Character.toLowerCase(character) + toSnakeCase(input, indexCount + 1);
        }
        else {
            return character + toSnakeCase(input, indexCount + 1);	
        }
    }
    // Recursive method to convert a snake_case string to camelCase
    public static String toCamelCase(String input, int indexCount, boolean capitalizeNext) {
        if (indexCount >= input.length()) {
            return "";
        }
        char character = input.charAt(indexCount);
        if (character == '_') {
            return toCamelCase(input, indexCount + 1, true);
        }
        else {
            if (capitalizeNext) {
                return Character.toUpperCase(character) + toCamelCase(input, indexCount + 1, false);
            }
            else {
                return character + toCamelCase(input, indexCount + 1, false);
            }
        }
    }
    /***
    ...
    public static int binaryToDecimal(String binaryString, int indexCount) {
        if (indexCount < 0) {
            return 0;
        }
        char digit = binaryString.charAt(indexCount);
        int digitValue = (digit == '1') ? 1 : 0;
        int powerOfTwo = 1 << (binaryString.length() - 1 - indexCount);
        return binaryToDecimal(binaryString, indexCount - 1) + (digitValue * powerOfTwo);
    }
}